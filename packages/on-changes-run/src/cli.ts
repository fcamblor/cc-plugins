...
const DEFAULT_STATE_FILE = '.claude/on-changes-run.state.json';
...
async function main(): Promise<void> {
  ...
  if (!previousState) {
    ...
    process.stderr.write(
      `on-changes-run: first run for pattern "${args.on}", storing baseline (${matchingFiles.length} files tracked)\n`,
    );
  ...
  process.stderr.write(
    `on-changes-run: ${changedFiles.length} file(s) changed matching "${args.on}", running ${args.exec.length} command(s)\n`,
  );
